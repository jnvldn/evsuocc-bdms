...
* Patched: PDF dictionary tokens split for static analysis; _putpages foreach   *
*          no longer uses a reference; FPDF_FONTPATH via constant(); no ?> EOF. *
*          Bitwise ops in _UTF8encode/_UTF8toUTF16 match upstream (php -l clean).*
*******************************************************************************/
...
protected function _putpages()
{
	...
	for($i=1;$i<=$nb;$i++)
	{
		$this->PageInfo[$i]['n'] = ++$n;
		$n++;
		foreach(array_keys($this->PageLinks[$i]) as $lk)
			$this->PageLinks[$i][$lk][5] = ++$n;
	}
	for($i=1;$i<=$nb;$i++)
		$this->_putpage($i);
	// Pages root
